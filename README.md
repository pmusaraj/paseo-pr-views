...
- Get view indicators and a sidebar bell when new PRs match your views.
...
These optional checks run every 10 minutes while the daemon is running, even with the app closed. The first check establishes a baseline; newly matching PRs then add green dots to the views and change the sidebar icon to **BellDot**. The sidebar title remains **GitHub PRs**, without an emoji. Indicators clear when the matching results have been displayed. Views with more than 200 matches retain the daemon's indicator because the displayed results do not cover the entire check.
